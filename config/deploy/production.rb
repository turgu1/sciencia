...
set :nginx_ssl_cert_local_path, "#{fetch(:rails_root)}/../nginx-selfsigned.crt"
set :nginx_ssl_cert_key_local_path, "#{fetch(:rails_root)}/../nginx-selfsigned.key"

server fetch(:server),
  user: fetch(:app_user),
  roles: %w{app db web},
  superuser: fetch(:superuser),
  primary: true
